...
queue = deque([(step, n_cost + (ord(step) - ord('A'))) for step in filter(lambda x: not x in deps, sorted(steps))])
...
while len(queue) > 0 or len(progress) > 0:
        ...
        if cost == 0:
            ...
            for dep in sorted(deps):
                if step in deps[dep]:
                    ...
                    if len(deps[dep]) == 0:
                        queue.append((dep, n_cost + (ord(dep) - ord('A'))))
    ...
    print seconds, [p for p in progress], string
    seconds += 1
